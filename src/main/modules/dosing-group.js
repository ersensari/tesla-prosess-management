...
const Repo = require("../database/repositories/dosing-group");
...
module.exports = async () => {
  ...
  ipcMain.on(MODULE_NAME + ".assignRaw", async (event, payload) => {
    try {
      await Repo.mutations.assignRawToSilo(
        payload.siloId,
        payload.rawMaterialId
      );
      event.reply(MODULE_NAME + ".assignRawCompleted");
    } catch (error) {
      event.reply(MODULE_NAME + ".assignRawError", {
        code: 505,
        message: error.message,
      });
    }
